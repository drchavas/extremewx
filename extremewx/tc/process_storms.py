...
# ---- write split, gzipped data (fetched + gunzipped in the browser) ----
# data/index.json.gz : small — meta, per-basin climatology, and a lightweight storm list
#                      (sid/name/year/basin/atcf, no track points) for the dropdowns.
# data/basin_<B>.json.gz : { sid: [points...] } for one basin, loaded on demand & cached.
...
index = {"meta": meta, "clim": clim,
         "storms": [{"sid": s["sid"], "name": s["name"], "year": s["year"],
                     "basin": s["basin"], "atcf": s["atcf"]} for s in cstorms]}
total = write_gz(os.path.join(DATADIR, "index.json.gz"), index)

by_basin = {}
for s in cstorms:
    by_basin.setdefault(s["basin"], {})[s["sid"]] = s["pts"]
for b, d in sorted(by_basin.items()):
    total += write_gz(os.path.join(DATADIR, f"basin_{b}.json.gz"), d)

# remove any stale single-file outputs so they aren't deployed
...
print(f"storms: {len(out)}  years: {years[0]}-{years[-1]}  basins: {len(by_basin)}  "
      f"total: {total/1e6:.1f} MB gz -> {DATADIR}/")
